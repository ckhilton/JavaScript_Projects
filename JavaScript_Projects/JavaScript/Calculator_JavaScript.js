...
function Input_Digit(digit) {
    ...
    //We are checking to see if 'Wait_Second_Operand' is true and set 'Display_Value
...
function Input_Decimal(dot) {
    //This ensures that accidental clicking of the decimal point doesn't cause
    //bugs to occur in the operation.
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //We are saying that if the 'Display_Value' does not contain a decimal point
        //that we want to add a decimal point.
...
function Handle_Operator(Next_Operator) {
    ...
    //on the screen to a number then store the result in 'Calculator.First_Operand' if
    ...
    if (First_Operand == null) {
        ...
        //If 'operator' already exists, the property lookup is performed for the operator
...
keys.addEventListener("click", (event) => {
    ...
    //that was clicked on.
    const { target } = event;
    //If the element that was clicked on is not a button, it exits the function.
    ...
    //This ensures that 'AC' actually clears the numbers from the Calculator.
